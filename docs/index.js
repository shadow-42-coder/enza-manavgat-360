...
(function() {
  ...
  (function setupSplashIntro() {
    ...
    // Admin (?pos=1) skips the full cinematic intro entirely - they open
    // this link constantly to make quick edits, and don't need to sit
    // through an ~11s sequence meant to make a first-time visitor's opening
    // moment memorable. Real visitors still get the whole thing.
    var isAdmin = /[?&]pos=1/.test(window.location.search);
    if (isAdmin) splashElement.classList.remove('playing');
    var reduced = isAdmin || (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
